...
void ContoursEngine_UniformiseSensSegments(Contour& li_Contour) {
    ...
    for (size_t i = 1; i < li_Contour.elements.size(); ++i) {
        ...
        if (currentElement.StartCpy2D.IsEqual(referencePoint, tolerance)) {
            ...
            std::cout << "SketchParams::Contours_UniformiseSensSegments : ERREUR sur le segment " << currentElement.primitiveId << std::endl;
#endif
        }
    }
...
void ContoursEngine::Contours_DisplayContour(const Contour& current) {
    ...
    std::cout << "\t\tisClosed : " << (current.isClosed ? "yes" : "no") << std::endl;
    std::cout << "\t\tisInternal : " << (current.isInternal ? "yes" : "no") << std::endl;
    std::cout << "\t\thasError : " << (current.hasError ? "YES" : "no") << std::endl;
    std::cout << "\t\tDiagnostic : " << current.diagnosticMessage << std::endl;
    ...
    std::cout << "\t\tisBounding : (" << xmin << "," << ymin << ")->(" << xmax << "," << ymax << ")" << std::endl;
    ...
    for (size_t j = 0; j < current.elements.size(); j++) {
        ...
        case ContoursPrimitiveType::Line:
            std::cout << "Line [ID: " << el.primitiveId << "] (" << el.StartCpy2D.X() << "," << el.StartCpy2D.Y() << ") to (" << el.StopCpy2D.X() << "," << el.StopCpy2D.Y() << ") - ";
            break;
        case ContoursPrimitiveType::Circle:
            std::cout << "Circle [ID: " << el.primitiveId << "] Center(" << el.CenterCpy2D.X() << "," << el.CenterCpy2D.Y() << ") Radius " << el.Radius;
            break;
        default:
