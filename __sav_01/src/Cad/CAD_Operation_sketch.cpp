...
TopoDS_Shape SketchParams::evaluate(const CAD_Document& doc) const {
    std::cout << "SketchParams::evaluate -> Début" << std::endl;
    ...
    if (topologyResult.hasErrors || topologyResult.validContours.empty()) {
        std::cerr << "\t❌ Erreur : Topologie du Sketch invalide ou vide." << std::endl;
        return TopoDS_Shape();
    }
    ...
    for (const auto& contour : topologyResult.validContours)
    {
        ...
        if (wireBuilder.IsDone()) {
            std::cout << "\t✅ BRepBuilderAPI_MakeWire OK" << std::endl;
            currentWire3D = wireBuilder.Wire();
        } else {
            // Premier diagnostic : Affichage de la raison exacte d'échec de MakeWire
            std::cerr << "\t⚠️ Échec BRepBuilderAPI_MakeWire. Raison : ";
            switch (wireBuilder.Error()) {
            case BRepBuilderAPI_WireDone:
                std::cerr << "OK" << std::endl; break;
            case BRepBuilderAPI_EmptyWire:
                std::cerr << "Wire Vide" << std::endl; break;
            case BRepBuilderAPI_DisconnectedWire:
                std::cerr << "Wire Disjoint (Écart entre sommets supérieur à la tolérance BRep)" << std::endl; break;
            case BRepBuilderAPI_NonManifoldWire:
                std::cerr << "Wire Non-Manifold (Auto-intersection / Bicyclette / Branloche)" << std::endl; break;
            default:
                std::cerr << "Erreur Inconnue (" << wireBuilder.Error() << ")" << std::endl; break;
            }
        ...
        std::cout << "\t[Diagnostic Contour " << (contour.isInternal ? "Intérieur" : "Extérieur")
    ...
    if (!outerWireCreated) {
        std::cerr << "\t❌ Erreur : Pas de contour extérieur principal." << std::endl;
        return TopoDS_Shape();
    }

    // 5. CONSTRUCTION DE LA FACE PLANE
    std::cout << "\tGénération de la Face plane avec évidements..." << std::endl;
    ...
    if (!faceMaker.IsDone()) {
        std::cerr << "\t❌ Erreur : BRepBuilderAPI_MakeFace a échoué." << std::endl;
        return outerWire3D;
    }
    ...
    std::cout << "\tFace créée avec succès." << std::endl;
    return faceFinale;
}
